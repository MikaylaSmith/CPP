...
void CommandHandle::Read()
{
	...
	***********/
	...
	ifstream inFile("Lab 4 Input.txt");

	if (inFile.is_open())
	{
		ofstream outFile("Lab 4 Output.txt");
		...
		if (outFile.is_open())
		{
			...
			switch (action)
			{
				...
				case 'P':
					...
					if (action == 'A')
					{
						//Confirmed to be the PA command
						//Also clears up what has and hasn't been read from inFile
						isam.Iterate();
					}
					break;
				case 'E':
			...
			if (outFile.is_open())
			{
				...
				switch (action)
				{
					...
					case 'P':
						...
						{
							//Leaves this open for Other command
						}
						break;
					case 'E':
						inFile.close();
